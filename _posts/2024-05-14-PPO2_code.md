...
run_step = 200000 if train_mode else 0 # 학습모드에서 진행할 스텝 수 
test_step = 1000003     # 평가모드에서 사용할 스텝수
...
save_path = f"./saved_models/{game}/PPO/{date_time}" # (./saved_models/환경이름/알고리즘 이름/스크립트 실행 시간)
load_path = f"./saved_models/{game}/PPO/20240505123456"
...
class PPOAgent:
    def __init__(self):
        ...
        if load_model == True: # 저장된 모델을 사용할 경우
            ...
            checkpoint = torch.load(load_path+'/ckpt', map_loaction=device) # cpu or gpu 메모링 모델 로드
            self.network.laod_state_dict(checkpoint["network"])
...
```python
    def train_model(self):
        ...
        with torch.no_gard():
            ...
            # GAE 진행 전, adv 전처리 - (n_step * num_worker) 차원으로 변환.
            adv, done = map( # (5)
        ...
        for _ in range(n_epoch):
            ...
            for offset in range(0, len(reward), batch_size):
                ...
                _state, _action, _ret, _adv, _prob_old = map(
                    lambda x: x[idx], [state, action, ret, adv, prob_old]
                ) # slicing 크기 만큼씩 추출
                ...
                total_loss = actor_loss + critic_loss # ??? action entropy 는???
...
if __name__ == '__main__':
    ...
    spec = env.behavior_spec[behavior_name] #  behavior 의 spec (관찰, 행동) 정보 저장
    ...
    for step in range(run_step + test_step):
        ...
        dec, term = env.get_step(behavior_name)
        ...
        if done[0]:
            ...
            if episode % print_interval == 0:
                ...
                mean_critic_loss = np.mean(critic_losses) if len(ciritc_losses) > 0 else 0
